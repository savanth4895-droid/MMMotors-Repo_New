...
async def verify_token(
    ...
    # Accept Bearer token from Authorization header (from localStorage) or httpOnly cookie
    token = mm_token
    try:
...
# ── Sequential counters ─────────────────────────────────────────────────────────
_PREFIX = {"invoice": ("INV", 6), "job": ("SRV", 6), "part_bill": ("PRT", 6), "part": ("PT", 6), "accident_estimate": ("EST", 4)}
